...
const Executives = require("../models/executiveModel")
...
const registerUser = asyncHandler(async (req, resp) => {
    ...
    const { username, email, password, confirmPassword, country, state, city } = req.body;
    ...
    if (!username || !email || !password || !confirmPassword || !country || !state || !city) {
        resp.status(400);
...
const loginUser = asyncHandler(async (req, resp) => {
    ...
    const executive = await Executives.findOne({ email: { $regex: new RegExp(`^${normalizedEmail}$`, 'i') } });
...
module.exports = { registerUser, loginUser, currentUser,getStateCity }