...
function EditSupplier({ isOpen, supplierId, onClose, onUpdate }) {
  ...
  useEffect(() => {
    ...
    const fetchSupplier = async () => {
      try {
        const response = await axios.get(`http://localhost:5001/api/supplier/${supplierId}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.put(`http://localhost:5001/api/supplier/${supplierId}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
