...
function EditProductStock({ isOpen, onClose, productStockId }) {
  ...
  useEffect(() => {
    const fetchProductStock = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5001/api/productStock/${productStockId}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.put(`http://localhost:5001/api/productStock/${productStockId}`, {
        ...formData,
