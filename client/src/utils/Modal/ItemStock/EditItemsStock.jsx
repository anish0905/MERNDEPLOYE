...
function EditItemsStock({ isOpen, onClose, itemStockId }) {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5001/api/itemStocks/${itemStockId}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.put(`http://localhost:5001/api/itemStocks/${itemStockId}`, {
        ...formData,
