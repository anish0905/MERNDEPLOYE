...
const SuperStockist = () => {
  ...
  useEffect(() => {
    const fetchSuperstockists = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5001/api/superStockistDetails"
        );
  ...
  return (
    <div className="flex bg-gray-200 h-screen">
      <Navbar />
      <div className="flex flex-1 flex-col ml-96 mr-5 ">
        <Logout />
        <Link to="/myOrders">My Order History</Link>
          ...
          {currentCards.map((superstockistGroup) =>
            superstockistGroup.shop_details.map((superstockist) => (
              <Card
                ...
                className="w-72 h-auto flex flex-col"
              >
                ...
                <CardContent className="flex-1">
                  <Typography variant="body1" gutterBottom>
                    ...
                    {superstockist.location}
                  </Typography>
                  <Typography>
                    <span className="font-semibold">State:</span>{" "}
                    {superstockistGroup._id}
                  </Typography>
                  <Typography variant="body1" gutterBottom>
                    <span className="font-semibold">Phone Number:</span>{" "}
                    {superstockist.phoneNumber}
                  </Typography>
                  <Typography variant="body1" gutterBottom>
                ...
                <Link
                  ...
                  className="bottom-0 left-0 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded text-center"
